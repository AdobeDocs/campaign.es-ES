...
[](enterprise-deployment.md) Batching operation is always preferred. In order to improve performance, ingestion APIs are redirected to the local database.
...
* New APIs dedicated for data ingestion flow directly into the local staging table. [Más información](new-apis.md)
* A scheduled workflow triggers every hour and synchronize data back to the Cloud Database. [Más información](replication.md)
...
1. Create a sample custom schema on Campaign Cloud database. No staging enabled at this step.
   ...
   ![](../assets/do-not-localize/glass.png)[](../dev/create-schema.md)

1. Save and update the database structure.  [Más información](../dev/update-database-structure.md)
