...
>[!NOTE]
>
>La variable **autouuid** el atributo solo se aplica a [Implementaciones de Enterprise (FFDA)](../architecture/enterprise-deployment.md).
...
En el contexto de un [Implementación empresarial (FFDA)](../architecture/enterprise-deployment.md), la clave principal de Adobe Campaign es un UUID generado automáticamente para todas las tablas integradas. También se puede utilizar un UUID para tablas personalizadas. [Más información](../architecture/keys.md)
...
Declarar un vínculo como una unión externa no es bueno para el rendimiento. El registro de id cero emula la funcionalidad de unión externa. En el contexto de un [Implementación empresarial (FFDA)](../architecture/enterprise-deployment.md), no es necesario declarar las uniones externas si el vínculo utiliza la variable **autouuid**.
